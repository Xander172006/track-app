                    ...
                    <div className="p-4 sm:p-8 text-white shadow sm:rounded-lg">
                        {!gameAccount ? (
                            ...
                            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 place-content-center'>
                                <div className='bg-black p-8 rounded-xl shadow-lg shadow-gray-800 w-[35rem]'><ActiveTab gameAccount={gameAccount} activeTab={activeTab} setActiveTab={setActiveTab} user={user}/></div>

                                {activeTab === 'gebruikersinformatie' && (
                                    <div className='bg-black p-0 rounded-xl shadow-lg shadow-gray-800 w-[35rem]'><GebruikersInformatie user={user} /></div>
                                )}
                                {activeTab === 'beveiliging' && (
                                    <div className='bg-black p-0 rounded-xl shadow-lg shadow-gray-800 w-[35rem]'><SecurityInformation user={user} /></div>
                                )}
                                {activeTab === 'gameAccountSettings' && (
                                    <div className='bg-black p-0 rounded-xl shadow-lg shadow-gray-800 w-[35rem]'><GameAccountStats activeTab={activeTab} user={user} gameAccount={gameAccount} success={success} error={error} auth={auth} /></div>
                                )}

                                <div className={`bg-black p-6 rounded-xl shadow-lg shadow-gray-800 w-[35rem] relative 
                                ...
                                `}><Salmonrungear SalmonrunApi={SalmonrunApi}/></div>
